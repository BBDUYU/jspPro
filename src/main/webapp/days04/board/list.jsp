...
<div>
  ...
  <script>
    ...
    $("#cmbNumberPerPage").on("change", function (){
    	let npp = $(this).val();
    	location.href = `<%= contextPath %>/cstvsboard/list.htm?currentPage=${pvo.currentPage}&numberPerPage=\${npp}`;
    });
    // 상태 관리
...
<script>
  $(".pagination a:not(.active)").attr("href", function (index, oldHref){
	  ...
	  return `<%= contextPath %>/cstvsboard/list.htm?currentPage=\${oldHref}&numberPerPage=\${npp}&searchCondition=\${sc}&searchWord=\${sw}`;
